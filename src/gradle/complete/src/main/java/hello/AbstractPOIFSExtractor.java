...
abstract class AbstractPOIFSExtractor {
    ...
    protected void handleEmbeddedResource(TikaInputStream resource, Metadata embeddedMetadata, String filename,
        ...
        try {
            ...
            if (relationshipID != null) {
                embeddedMetadata.set(EMBEDDED_RELATIONSHIP_ID, relationshipID);
            }
            if (storageClassID != null) {
                embeddedMetadata.set(EMBEDDED_STORAGE_CLASS_ID, storageClassID.toString());
            }
            if (mediaType != null) {
    ...
    protected void handleEmbeddedOfficeDoc(
        ...
        metadata.set(EMBEDDED_RELATIONSHIP_ID, dir.getName());
        if (dir.getStorageClsid() != null) {
            metadata.set(EMBEDDED_STORAGE_CLASS_ID, dir.getStorageClsid().toString());
        }
            ...
            if (type == POIFSDocumentType.OLE10_NATIVE) {
                try {
                    ...
                    if (ole.getLabel() != null) {
                        metadata.set(RESOURCE_NAME_KEY, rName + '/' + ole.getLabel());
                    }
                ...
                try {
                    ...
                    metadata.set(RESOURCE_NAME_KEY, rName + extension);
                } catch (Exception e) {
                    EmbeddedDocumentUtil.recordEmbeddedStreamException(e, parentMetadata);
                    return;
                }
                ...
                metadata.set(RESOURCE_NAME_KEY, rName + '.' + type.getExtension());
            }
